...
COR_VAZANTE = (0, 150, 136)      # Teal / Verde Água (Cor da Esperança)

def obter_cor_fundo(nivel, em_recessao):
    # Se estiver em recessão e o nível não for catastrófico (>900), usa a cor da esperança
    if em_recessao and nivel < 900:
        return COR_VAZANTE
...
def gerar_capa(dados_rio, tendencia, hist_2020, hist_2022, velocidade, em_recessao):
    ...
    # 3. Desenhar Texto: SITUAÇÃO (Com a nova lógica de texto)
...
# ==============================================================================
# FUNÇÃO 2: GERAR PLACAR DAS RUAS (Mantida igual, só precisamos garantir que está aqui)
# ==============================================================================
def gerar_placar(risco_ruas):
    ...
    try:
        ...
        font_rua = ImageFont.truetype("arial.ttf", 55)
    ...
    POS_X_NOME = 50
    POS_X_BARRA = 530
    LARGURA_MAX_BARRA = 360
    POS_X_PORCENTAGEM = 910
    ALTURA_BARRA = 40
    ...
    for rua in risco_ruas:
        ...
        draw.rectangle([(POS_X_BARRA, y_pos + 10), (POS_X_BARRA + LARGURA_MAX_BARRA, y_pos + 10 + ALTURA_BARRA)], fill=(50,50,50))
        
        pct_visual = min(pct, 100.0)
        largura_atual = (pct_visual / 100) * LARGURA_MAX_BARRA
        draw.rectangle([(POS_X_BARRA, y_pos + 10), (POS_X_BARRA + largura_atual, y_pos + 10 + ALTURA_BARRA)], fill=cor_barra)
...
# ==============================================================================
# ORQUESTRADOR (Atualizado com o parâmetro em_recessao)
# ==============================================================================
def gerar_todas_imagens(dados_rio, dados_ruas, tendencia, h2020, h2022, velocidade, em_recessao=False):
    caminho1 = gerar_capa(dados_rio, tendencia, h2020, h2022, velocidade, em_recessao)
